...
export const parseManyToMany = (
  ...
  const isEffectivelyBiDirectional = Boolean(
    relationship.bi_directional || inverseDefinition?.bi_directional
  );
  ...
  let isOwningSide: boolean | undefined =
    relationship.joinTableName ||
    relationship.joinColumnName ||
    relationship.inverseJoinColumnName
      ? true
      : undefined;
  ...
  if (isOwningSide === undefined && inverseDefinition) {
    ...
    isOwningSide = !(
      inverseDefinition.joinTableName ||
      inverseDefinition.joinColumnName ||
      inverseDefinition.inverseJoinColumnName
    );
  }
  ...
  const currentPropertyName =
    relationship.to_name || camelCase(pluralize(relationship.to));
  ...
  if (!currentPropertyName) {
    console.warn(
      `[APSO WARNING] Missing 'to_name' for ManyToMany: ${relationship.from} -> ${relationship.to}. Skipping this definition.`
    );
    return {};
  }
  ...
  if (isEffectivelyBiDirectional && !inversePropertyName) {
    console.info(
      `[APSO INFO] No 'to_name' provided for bidirectional ManyToMany: ${relationship.to} -> ${relationship.from}. Using default inverse property name. Add 'to_name' for explicit control.`
    );
  ...
  const fromSideRelationship: Relationship = {
    ...
    joinTableName: isOwningSide
      ? relationship.joinTableName || undefined
      : undefined, // Use current def details if owner
    joinColumnName: isOwningSide
      ? relationship.joinColumnName || undefined
      : undefined,
    inverseJoinColumnName: isOwningSide
      ? relationship.inverseJoinColumnName || undefined
      : undefined,
    inverseReferenceName: isEffectivelyBiDirectional
      ? toSideRefName
      : undefined, // Only add inverse name if bidirectional
  };
  ...
  if (isEffectivelyBiDirectional) {
    ...
    const toSideRelationship: Relationship = {
      ...
      joinTableName: isInverseOwningSide
        ? inverseDefinition?.joinTableName || undefined
        : undefined,
      joinColumnName: isInverseOwningSide
        ? inverseDefinition?.joinColumnName || undefined
        : undefined,
      inverseJoinColumnName: isInverseOwningSide
        ? inverseDefinition?.inverseJoinColumnName || undefined
        : undefined,
...
export const parseRelationship = (
  ...
  switch (relationship.type) {
        ...
        [relationship.from]: [
          {
            ...
            biDirectional: true,
            referenceName: fromSideRefName,
            inverseReferenceName: toSideRefName,
          },
        ],
        [relationship.to]: [
          {
            ...
            index: relationship.index || false,
            cascadeDelete: relationship.cascadeDelete || false,
            referenceName: toSideRefName,
            inverseReferenceName: fromSideRefName,
          },
        ],
      };
    }
    ...
    case "OneToOne": {
      ...
      const fromRel = createFromOneToOneRelationship(
        relationship,
        inverseOnetoOneDef
      );
      ...
      if (isEffectivelyBiDirectional) {
        ...
        if (inverseOnetoOneDef) {
          ...
          const toRel = createToOneToOneRelationship(
            relationship,
            inverseOnetoOneDef,
            isFromSideOwner
          );
          ...
          console.warn(
            `[APSO WARNING] Bidirectional OneToOne defined for ${relationship.from} -> ${relationship.to}, but inverse definition ${relationship.to} -> ${relationship.from} is missing. Inverse side generation may be incomplete.`
          );
...
export const getRelationshipForTemplate = (
  ...
  relationships.forEach((relationship: Relationship) => {
    ...
    if (
      !processedRefs.has(key) ||
      (relationship.join && !processedRefs.get(key)?.join)
    ) {
  ...
  return [...processedRefs.values()].map((relationship: Relationship) => {
    ...
    const needsPluralInverse =
      relationship.type === "ManyToOne" || relationship.type === "ManyToMany";
    ...
    if (
      ...
      !relationship.inverseReferenceName &&
      (relationship.type === "ManyToMany" ||
        relationship.type === "OneToMany" ||
        relationship.type === "OneToOne")
    ) {
      console.info(
        `[APSO INFO] Using default property name '${inverseSidePropertyName}' for ${relationship.name}->${entityName}. To customize, add 'to_name' in your relationship definition.`
      );
    }
    ...
    return {
      ...relationship,
      relationshipName,
      pluralizedRelationshipName: camelCase(
        pluralize(thisReferenceName || relationship.name)
      ),
      pluralizedName: camelCase(pluralize(relationship.name)),
...
export function* generateNestedRelationships(
  ...
  for (const relationship of relationships) {
    ...
    const isImmediateSelfLoop =
      relationship.name === entityName &&
      path.length > 0 &&
    ...
    if (relationship.name === baseEntity) {
      ...
      const isSpecificFunctionalAlias =
...
export function getNestedRelationships(
  ...
  return [
    ...generateNestedRelationships(entityName, relationshipMap, maxDepth),
  ];
}
...
export async function streamNestedRelationships(
  ...
  let count = 0;
  for (const joinPath of generateNestedRelationships(
    entityName,
    relationshipMap,
    maxDepth
  )) {
