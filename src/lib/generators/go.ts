...
export class GoGenerator extends BaseGenerator {
  ...
  async generateMigration(
    ...
    const upStatements = upSql.map((sql) => `${sql};`).join("\n");
    const downStatements = (downSql || []).map((sql) => `${sql};`).join("\n");
...
`;
  ...
  async generateIndexModule(
    ...
    const modelRefs = entities
      ...
      .join("\n");
    const registryContent = `package models

// All returns all registered GORM models for AutoMigrate.
// This file is auto-generated by the Apso CLI.
func All() []interface{} {
\treturn []interface{}{
${modelRefs}
\t}
